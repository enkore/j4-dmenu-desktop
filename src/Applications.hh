...
// a helper function that safely gets an iterator from container.insert() and container.emplace()
...
{
private:
    // This struct holds iterators to sortedentrymap members and a reference to desktop file ID which
    // is useful in update_log().
    struct histentry
    {
        // These iterators point to a valid element only if the name is currently displayed i.e.
        ...
        {
            return iter == other.iter && generic_iter == other.generic_iter && id == other.id;
        }
    };
    ...
    struct namelistentrytype
    {
        ...
        bool operator<(const namelistentrytype & other) const { return app->hist < other.app->hist; }
        bool operator==(const namelistentrytype & other) const { return app == other.app && isgeneric == other.isgeneric; }
    };

    // The primary struct.
    // All data about a desktop file is directly or indirectly accessible through this.
    struct appcontainer
    {
        // Precedence of the desktop file based on the SearchPath order.
        // When two desktop files have the same ID, the one with lower rank value has priority.
        int rank;
        Application app;
        struct
        {
        ...
        {
            return rank == other.rank && app == other.app && hist == other.hist && display == other.display;
        }
    };

    // None of these four containers shall have an empty container as their value, because that would
    // break things. Applications must take care to remove the entire key value pair when it's removing
    // the last entry from value. For example when there are two names belonging to one desktop entry
    // in sortedentrymap with history 1, the sortedentrymap[1] must be removed altogether when the
    // desktop entry's history increases because empty std::set<string_ref> would break sortediter.

    std::unordered_map<std::string /* ID */, appcontainer> idmap;

    // This map contains all Names and GenericNames of all programs as keys and the desktop entries
    // with that name as values. This prevents duplicate names.
    // Each name can have one or more namelistentrytypes attached to it, but only the last one
    // (the one for which isdisplayed() returns true) is active.
    ...
    // Beware, a desktop entry doesn't have to have both Name and GenericName displayed because
    // there could be a desktop entry with higher history and with the same Name or GenericName. Higher
    // history has higher priority. Both Name and GenericName can be shadowed.
    std::map<unsigned, std::set<string_ref>, std::greater<unsigned>> sortedentrymap;
    ...
    bool isdisplayed(const std::multiset<namelistentrytype> & list, const std::multiset<namelistentrytype>::const_iterator & listiter) const
    {
    ...
    {
        if (isgeneric)
            return isdisplayed(namelist.at(app.app.generic_name), app.listcandidate.generic_iter);
        else
            return isdisplayed(namelist.at(app.app.name), app.listcandidate.iter);
    }

    void add_generic(appcontainer & appc, bool isgeneric)
    {
        const std::string & name = isgeneric ? appc.app.generic_name : appc.app.name;
        std::set<string_ref>::const_iterator & histiter = isgeneric ? appc.listed->generic_iter : appc.listed->iter;
        std::multiset<namelistentrytype>::const_iterator & namelistentry = isgeneric ? appc.listcandidate.generic_iter : appc.listcandidate.iter;
        ...
        if (listentries == namelist.end()) {
            ...
            if (isdisplayed(appc, isgeneric)) { // we are currently displayed, undisplay the old entry
                auto & orig = *std::prev(namelistentry);
                if (orig.isgeneric)
                    sortedentrymap.at(orig.app->hist).erase(orig.app->listed->generic_iter);
                else
                    sortedentrymap.at(orig.app->hist).erase(orig.app->listed->iter);
    ...
    {
        const std::string & name = isgeneric ? appc.app.generic_name : appc.app.name;
        std::set<string_ref>::const_iterator & sortediter = isgeneric ? appc.listed->generic_iter : appc.listed->iter;
        const std::multiset<namelistentrytype>::const_iterator & listiter = isgeneric ? appc.listcandidate.generic_iter : appc.listcandidate.iter;
        ...
        if (namelist.at(name).size() == 1)
        {
            ...
            namelist.erase(name); // delete the entire namelist entry rather than having it with an empty list
        }
        else if (isdisplayed(namelist.at(name), listiter))
        {
            sortedentrymap.at(appc.hist).erase(sortediter); // delete listed entry of name
            if (sortedentrymap.at(appc.hist).empty())
                sortedentrymap.erase(appc.hist);
            namelist.at(name).erase(listiter);
            auto & newentry = *std::prev(namelist.at(name).end()); // replace it with the next suitable entry
            if (!exclude_generic && newentry.isgeneric)
                 newentry.app->listed->generic_iter = getiter(sortedentrymap[newentry.app->hist].insert(newentry.app->app.generic_name));
            else
                 newentry.app->listed->iter = getiter(sortedentrymap[newentry.app->hist].insert(newentry.app->app.name));
        }
        else
            namelist.at(name).erase(listiter);
    }
    ...
    {
        std::set<string_ref>::const_iterator & sortediter = isgeneric ? appc.listed->generic_iter : appc.listed->iter;
        std::multiset<namelistentrytype>::const_iterator & nameiter = isgeneric ? appc.listcandidate.generic_iter : appc.listcandidate.iter;
        const std::string & name = isgeneric ? appc.app.generic_name : appc.app.name;
        ...
        if (isdisplayed(appc, isgeneric))
        {
            if (namelist.at(name).size() != 1)
            {
                ...
                if (!exclude_generic && undisplay.isgeneric)
                    sortedentrymap.at(undisplay.app->hist).erase(undisplay.app->listed->generic_iter);
                else
                    sortedentrymap.at(undisplay.app->hist).erase(undisplay.app->listed->iter);
    ...
    void sethistory(appcontainer & appc, unsigned newval)
    {
        ...
        appc.listed = sortedhistmap[newval].insert(sortedhistmap[newval].end(), appc.app.id);
    ...
    // This iterator crawls through sortedentrymap and mapped_type and presents it continuously.
    class sortediter
    {
        private:
            decltype(sortedentrymap)::const_iterator topiter;
            std::set<string_ref>::const_iterator nameiter;
            // When topiter is incremented, we can't just set nameiter to topiter->second.begin()
            // because the newly incremented topiter might be the past-the-end iterator and therefore
            // undeferencable. When topiter is incremented, isunknown is set to true and the next time
            // someone deferences or increments sortediter, we assume that topiter is deferencable and
            // set nameiter.
        ...
        public:
            ...
            {
                return topiter == other.topiter && (isunknown == other.isunknown && (isunknown || nameiter ==
                        other.nameiter));
            }
            bool operator!=(const sortediter & other) const
            {
            ...
            sortediter() = default;
            sortediter(decltype(topiter) t, decltype(nameiter) n) : topiter(t), nameiter(n) {}
...
public:
    Applications(application_formatter f, const stringlist_t & denvs, const LocaleSuffixes & suff, bool exclude, bool history) : format(f), suffixes(suff), desktopenvs(denvs), exclude_generic(exclude), use_history(history) {}

    /// These functions and operators don't have much use in j4dd, but are useful for tests.

    Applications(const Applications & other) : format(other.format), suffixes(other.suffixes), desktopenvs(other.desktopenvs), exclude_generic(other.exclude_generic), use_history(other.use_history)
    {
        for (const auto & i : other.idmap)
        {
            ...
            appc.listed = sortedhistmap[appc.hist].insert(sortedhistmap[appc.hist].end(), curr.first);
            if (other.isdisplayed(i.second, false))
                appc.listed->iter = getiter(sortedentrymap[appc.hist].insert(appc.app.name));
            if (!exclude_generic && !appc.app.generic_name.empty())
            {
                namelist[appc.app.generic_name].emplace(&appc, true);
                if (other.isdisplayed(i.second, true))
                    appc.listed->generic_iter = getiter(sortedentrymap[appc.hist].insert(appc.app.generic_name));
            }
        }
    }
    ...
    {
        return format == other.format && suffixes == other.suffixes && desktopenvs == other.desktopenvs
            && exclude_generic == other.exclude_generic && use_history == other.use_history && idmap == other.idmap
    ...
    sortediter begin() const
    {
        if (sortedentrymap.empty())
            ...
            return sortediter(sortedentrymap.cbegin(), sortedentrymap.cbegin()->second.cbegin());
    }
    ...
    void remove(const std::string & filename, int rank)
    {
        ...
        std::replace_copy(filename.cbegin(), filename.cend(), std::back_inserter(id), '/', '-');
    ...
    std::pair<Application *, std::string> get(const std::string &choice) {
        ...
        if(match_length) {
            ...
            return std::make_pair(&app->app, choice.substr(match_length, choice.length()-1));
        }
    ...
    // If the log doesn't exist, than do nothing. A next update_log() call should create it.
    ...
    void update_log(const char *log) {
        ...
        for (auto x = sortedhistmap.crbegin(); x != sortedhistmap.crend() && x->first != 0; ++x) {
