...
TEST_CASE("Notify/create_delete", "Test detection of file creation and deletion of a subdirectory of watched directory")
{
    ...
    // Try to delete FILENAME if it exists. A failed test might not have properly deleted it, so this assures that it won't interfere with tests.
    ...
    // Timeout of 65 seconds is set because NotifyKqueue polls events in one minute intervals.
    // NotifyInotify should return immediately on these polls.
    ...
    // since the event was handled, there should be no other events available therefore notify.getfd()
