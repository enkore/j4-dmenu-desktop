...
// This function will pass the string to the shell, the shell will unquote and expand it and it will
...
stringlist_t getshell(const std::string & args)
{
    ...
    switch (pid = fork())
    {
        ...
        case 0:
            ...
            close(pipefd[1]);
            close(STDIN_FILENO);
            execl("/bin/sh", "/bin/sh", "-c", ((std::string)"printf '%s\\0' " + args).c_str(), (char *) NULL);
            _exit(1);
    }
    close(pipefd[1]);
    int status;
...
TEST_CASE("ApplicationRunner/expand", "Tests proper handling of special characters")
{
    LocaleSuffixes ls("en_US");
    char * data = nullptr;
    size_t size;
    Application app(TEST_FILES"applications/gimp.desktop", &data, &size, appformatter_default, ls, {});
...
TEST_CASE("ApplicationRunner/field_codes", "")
{
    ...
    char * data = nullptr;
    size_t size;
    Application app(TEST_FILES"applications/field_codes.desktop", &data, &size, appformatter_default, ls, {});

    auto result = getshell(application_command(app, "arg1 arg2\\ arg3"));
    stringlist_t cmp({ "true", "--name=%c", "--location", TEST_FILES "applications/field_codes.desktop", "arg1", "arg2\\", "arg3" });
    REQUIRE( result == cmp );
...
TEST_CASE("ApplicationRunner/escape", "Regression test for issue #18, %c was not escaped")
{
    ...
    char * data = nullptr;
    size_t size;
    Application app(TEST_FILES"applications/caption.desktop", &data, &size, appformatter_default, ls, {});
