...
TEST_CASE("Application/get", "")
{
    ...
    auto query = apps.get("true"); // requested direct executable instead of desktop entry
    ...
    // a incorrect implementation could assume that the requested desktop entry is "Web" and
    // the arguments are "browser --help". This verifies correct behaviour.
...
TEST_CASE("Applications/comparison", "")
{
    ...
    stringlist_t cmp({ "Eagle", "GNU Image Manipulation Program", "Htop", "Image Editor", "Process Viewer" });
...
TEST_CASE("Applications/bad_remove", "Test removing nonexistent/invalid desktop entries")
{
    Applications apps(appformatter_default, {}, {}, false, false);
    ...
    // trying to remove eagle.desktop which is in a different SearchPath directory than the current desktop entry
...
TEST_CASE("Applications/basic_colision", "Test desktop ID precedence")
{
    SECTION("the former overrides the latter") {
        // This simulates $XDG_DATA_DIRS="TEST_FILESusr/share:TEST_FILESusr/local/share"
        Applications apps(appformatter_default, {}, {}, false, false);

        REQUIRE_NOTHROW(apps.add("collision.desktop", 0, TEST_FILES"usr/share/applications/"));
        REQUIRE(std::distance(apps.begin(), apps.end()) == 1);
        REQUIRE(*apps.begin() == "First");

        REQUIRE_NOTHROW(apps.add("collision.desktop", 1, TEST_FILES"usr/local/share/applications/"));
        REQUIRE(std::distance(apps.begin(), apps.end()) == 1);
        REQUIRE(*apps.begin() == "First");
    }
    SECTION("the latter overrides the former") {
        // This simulates $XDG_DATA_DIRS="TEST_FILESusr/local/share:TEST_FILESusr/share"
        Applications apps(appformatter_default, {}, {}, false, false);

        REQUIRE_NOTHROW(apps.add("collision.desktop", 1, TEST_FILES"usr/share/applications/"));
        REQUIRE(std::distance(apps.begin(), apps.end()) == 1);
        REQUIRE(*apps.begin() == "First");

        REQUIRE_NOTHROW(apps.add("collision.desktop", 0, TEST_FILES"usr/local/share/applications/"));
        REQUIRE(std::distance(apps.begin(), apps.end()) == 1);
        REQUIRE(*apps.begin() == "Second");
    }
}

TEST_CASE("Applications/increment_history", "Verify that get()ing a Application increments its history")
{
    Applications apps(appformatter_default, {}, {}, false, true);

    REQUIRE_NOTHROW(apps.add("firefox.desktop", 0, TEST_FILES"a/applications/"));

    REQUIRE_NOTHROW(apps.add("safari.desktop", 1, TEST_FILES"b/applications/"));
...
TEST_CASE("Applications/disable_increment_history", "Verify that get()ing a Application doens't have side effects when history is turned off")
{
    Applications apps(appformatter_default, {}, {}, false, false);
...
TEST_CASE("Applications/shadow", "Test correct handling of desktop files with overlapping names")
{
    Applications apps(appformatter_default, {}, {}, false, true);
    ...
    // getting "Eagle" twice tests correct history handling when the group of desktop entries
    // with history 1 gets completely moved to the "history group" 2 (this tests that the histentries with
    // hist value 1 get properly deleted in favour of hist 2)
    apps.get("Eagle");
    REQUIRE(std::distance(apps.begin(), apps.end()) == 1);
...
TEST_CASE("Applications/exclude_generic", "")
{
    ...
    stringlist_t cmp({ "Eagle", "GNU Image Manipulation Program", "Htop", "visibleApp" });
    REQUIRE(std::equal(apps.begin(), apps.end(), cmp.begin()));
}
...
TEST_CASE("Applications/precedence", "")
{
    ...
    REQUIRE_NOTHROW(apps.add("hidden.desktop", 0, TEST_FILES"a/applications/"));
    REQUIRE(std::distance(apps.begin(), apps.end()) == 0);
    ...
    REQUIRE_NOTHROW(apps.add("chromium.desktop", 0, TEST_FILES"a/applications/"));
    REQUIRE(std::distance(apps.begin(), apps.end()) == 4);

    stringlist_t cmp({ "Chrome based browser", "Chromium", "Firefox", "Web browser" });
    REQUIRE(std::equal(apps.begin(), apps.end(), cmp.begin()));

    REQUIRE_NOTHROW(apps.add("safari.desktop", 1, TEST_FILES"b/applications/"));
    REQUIRE(std::distance(apps.begin(), apps.end()) == 5);

    cmp = { "Chrome based browser", "Chromium", "Firefox", "Safari", "Web browser" };
    REQUIRE(std::equal(apps.begin(), apps.end(), cmp.begin()));
    ...
    REQUIRE_NOTHROW(apps.add("chrome.desktop", 1, TEST_FILES"b/applications/"));
    REQUIRE(std::distance(apps.begin(), apps.end()) == 6);

    cmp = { "Chrome", "Chrome based browser", "Chromium", "Firefox", "Safari", "Web browser" };
    REQUIRE(std::equal(apps.begin(), apps.end(), cmp.begin()));

    REQUIRE_NOTHROW(apps.add("vivaldi.desktop", 2, TEST_FILES"c/applications/"));

    REQUIRE(std::distance(apps.begin(), apps.end()) == 7);

    cmp = { "Chrome", "Chrome based browser", "Chromium", "Firefox", "Safari", "Vivaldi", "Web browser" };
    REQUIRE(std::equal(apps.begin(), apps.end(), cmp.begin()));

    std::pair<Application *, std::string> bravesearch4 = apps.get("Web browser");
    ...
    cmp = { "Chrome", "Chrome based browser", "Chromium", "Firefox", "Vivaldi", "Web browser" };
    REQUIRE(std::equal(apps.begin(), apps.end(), cmp.begin()));
    std::pair<Application *, std::string> bravesearch5 = apps.get("Web browser");
    ...
    cmp = { "Chrome", "Chrome based browser", "Chromium", "Firefox", "Web browser" };
    std::pair<Application *, std::string> bravesearch6 = apps.get("Web browser");
    REQUIRE(std::equal(apps.begin(), apps.end(), cmp.begin()));
    ...
    std::pair<Application *, std::string> bravesearch7 = apps.get("Chrome based browser");
    REQUIRE(std::equal(apps.begin(), apps.end(), cmp.begin()));
...
TEST_CASE("Applications/history", "Test loading and saving log")
{
    ...
    stringlist_t cmp({ "Eagle", "GNU Image Manipulation Program", "Image Editor" });
    REQUIRE(std::equal(apps.begin(), apps.end(), cmp.begin()));
    ...
    {
        WARN("Couldn't create a test history file. This could be caused by programming error or by insuffitient permissions to create " TEST_FILES "/testhist.");
    }
    else
    {
        Applications histapps(appformatter_default, {}, {}, false, true);
        REQUIRE_NOTHROW(histapps.add("eagle.desktop", 0, TEST_FILES"applications/"));
        REQUIRE_NOTHROW(histapps.add("gimp.desktop", 0, TEST_FILES"applications/"));
        ...
        if (unlink(TEST_FILES "/testhist") == -1)
            FAIL("Couldn't remove temporary history file " TEST_FILES "/testhist: " << strerror(errno));
    }
}

TEST_CASE("Applications/empty_log", "Test that handling a nonexistant log doesn't break Applications")
{
    // This removes log if it wasn't removed by the test (this can happen if the test would fail before calling the last unlink()).
    ...
    REQUIRE_NOTHROW(apps.add("eagle.desktop", 0, TEST_FILES"applications/"));
    auto query = apps.get("Eagle"); // Increase the history of eagle.desktop so that update_log() will have something to write.
